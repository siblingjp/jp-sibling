...
<template>
  <div class="max-w-xl">
    <div class="flex items-center gap-3 mb-6">
      ...
      <h1 class="text-2xl font-bold text-gray-900">ผู้ใช้งานใหม่</h1>
    </div>
    ...
    <form class="bg-white rounded-xl shadow-sm p-6 space-y-5" @submit.prevent="handleSubmit">
      <div>
        ...
        <input v-model="form.name" type="text" required class="input" placeholder="ชื่อ-นามสกุล" />
      </div>
      ...
      <div>
        ...
        <input v-model="form.password" type="password" required minlength="8" class="input" placeholder="อย่างน้อย 8 ตัวอักษร" />
      </div>
      ...
      <div class="flex gap-3 pt-2">
        <button type="submit" :disabled="isLoading" class="btn-primary">
          {{ isLoading ? 'กำลังบันทึก...' : 'สร้างผู้ใช้งาน' }}
        </button>
